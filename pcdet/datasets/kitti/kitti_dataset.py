...
class KittiDataset(DatasetTemplate):
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            if self.dataset_cfg.get('ADJUST_ROT', None): 
                # adjust center location
                noise = np.random.randn() 
                gt_boxes_lidar[:, 2] -= 1.2
                gt_boxes_lidar[:, :3] = self.rotate(gt_boxes_lidar, noise)
                # adjust z correction
                # gt_boxes_lidar[:, 2] += 0.22 * gt_boxes_lidar[:, 0]
            input_dict.update({
                'gt_names': gt_names,
        ...
        if "points" in get_item_list:
            ...
            if self.dataset_cfg.get('ADJUST_ROT', None): 
                # points = self.adjust_z(points)
                points[:, 2] -= 1.2 
                points[:, :3] = self.rotate(points, noise)
